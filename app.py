...
def init_products_table():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS products (
        ...
        brand TEXT,
        brand_en TEXT,
        has_collections TEXT,
        translated_menu TEXT,
        updated_price INTEGER,
        keywords JSONB,
        arabic_keywords JSONB,
        is_active BOOLEAN,
        picture TEXT,
        search_text TEXT
    )''')
...
def import_products_from_csv():
    ...
    if c.fetchone()[0] > 0:
        print(f"Products already imported: {c.fetchone()[0]} products")
        conn.close()
        return
    
    print("Importing products from CSV...")
    with open('FinalSheetFourtyK - Sheet1.csv', 'r', encoding='utf-8') as f:
        reader = csv.DictReader(f)
        count = 0
        for row in reader:
            try:
                # Parse price
                price_str = row.get('Price', '0').replace(',', '').replace('د.ع', '').strip()
                try:
                    price = int(float(price_str))
                except:
                    price = 0
                
                updated_price_str = row.get('Updated Price', '0').replace(',', '').strip()
                try:
                    updated_price = int(float(updated_price_str))
                except:
                    updated_price = price
                
                # Build search text
                search_text = f"{row.get('Name', '')} {row.get('Secondary Name', '')} {row.get('brand', '')} {row.get('Brand', '')} {row.get('Menu', '')} {row.get('translated Menu', '')} {row.get('Rephrased Description (EN)', '')}"
                
                c.execute('''INSERT INTO products (
                    item_id, name_ar, name_en, barcode, price, description_ar, description_en,
                    menu, brand, brand_en, has_collections, translated_menu, updated_price,
                    keywords, arabic_keywords, is_active, picture, search_text
                ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)''',
                (
                    row.get('Id'), row.get('Name'), row.get('Secondary Name'), row.get('Barcode'),
                    price, row.get('Description'), row.get('Secondary Description'),
                    row.get('Menu'), row.get('Brand'), row.get('Secondary Brand'),
                    row.get('Has Collections'), row.get('Translated Menu'), updated_price,
                    row.get('Keywords'), row.get('Arabic Keywords'),
                    row.get('Is Active', 'TRUE') == 'TRUE', row.get('Picture'), search_text
                ))
                count += 1
                if count % 5000 == 0:
                    conn.commit()
...
def get_categories():
    ...
    c.execute('''SELECT DISTINCT 
                 CASE 
                     WHEN menu LIKE '%,%' THEN split_part(menu, ',', 1)
                     ELSE menu
                 END as category
                 FROM products 
                 WHERE is_active = true AND menu IS NOT NULL
                 ORDER BY category''')
...
def get_brands():
    conn = get_db()
    c = conn.cursor()
    c.execute('''SELECT DISTINCT brand FROM products 
...
def get_products_api():
    try:
        ...
        if search:
            results, total = search_products(search, limit, offset)
        else:
            results, total = get_all_products(limit, offset, category, brand)
...
# SaleBorg AI Chat API
SALEBORG_PROMPT = """You are SaleBorg, an expert beauty and cosmetics product consultant. Your job is to help customers find the perfect products.

ABOUT THE STORE:
- We have 40,000+ premium beauty & cosmetics products
- Brands include: Chanel, Pupa Milano, Marc-Antoine Barrois Paris, Ayten, and many more
- Products: perfumes, makeup, skincare, haircare
- Price range: budget to luxury (5,000 - 300,000+ IQD)
- Language: You can respond in both English and Arabic

YOUR JOB:
1. Understand what the customer is looking for
2. Search and recommend suitable products from the catalog
3. Ask follow-up questions to narrow down choices
4. Provide helpful information about products (price, description, brand)
...
- Format your response to include natural language that will trigger product search

Example:
User: "I want a perfume"
Response: "Great choice! Let me find some amazing perfumes for you! What type do you prefer - something fresh and citrusy, or warm and woody? And what's your budget range?" + [SEARCH: perfume]

User: "Show me lip gloss under 20000"
Response: "Here are some amazing lip glosses under 20,000 IQD! These are our top sellers..." + [SEARCH: lip gloss price<20000]"""
...
def saleborg_chat():
    ...
    try:
        ...
        formatted_messages = [
            {'role': 'system', 'content': SALEBORG_PROMPT},
            *cleaned_messages
        ]
        ...
        if response.status_code != 200:
            print(f'SaleBorg Groq API error: {response.status_code} - {response.text}')
        ...
        # Extract search query from response and search products
        products = []
        search_terms = extract_search_terms(assistant_message)
        
        if search_terms:
            try:
                search_results, _ = search_products(search_terms, limit=5, offset=0)
        ...
        print('SaleBorg Request timeout')
        return jsonify({'error': 'Request timed out. Please try again.'}), 500
    except Exception as e:
        print(f'SaleBorg Chat error: {e}')
        return jsonify({'error': 'Failed to process request'}), 500

def extract_search_terms(message):
    """Extract product search terms from AI response"""
    import re
    ...
    # Fallback: extract key nouns from the message
    keywords = ['perfume', 'lipstick', 'mascara', 'serum', 'cream', 'shampoo', 
                'conditioner', 'oil', 'gloss', 'makeup', 'skincare', 'hair', 'face',
                'body', 'eyes', 'lips', 'eyebrow', 'men', 'women', 'luxury', 'budget']
...
if __name__ == '__main__':
    # Initialize products table and import data
    try:
        init_products_table()
        import_products_from_csv()
    except Exception as e:
        print(f"Products initialization error: {e}")
